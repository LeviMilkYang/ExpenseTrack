...
- `bot_runtime/telegram_record_schema.py`
  `codex exec --output-schema` 用的输出 schema。新增账本字段时，这里通常也要同步改。
...
6. `telegram_codex_bridge.py apply` 补默认字段并标准化
7. `append_excel_entry.py` 调 helper 写入 Excel
